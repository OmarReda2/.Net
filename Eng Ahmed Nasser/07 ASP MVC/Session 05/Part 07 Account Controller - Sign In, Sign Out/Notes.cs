...
/*
a) Register/SignIn:
- "UserManager" is resposible for Creating User,"UserManager" in database:AspNetUsers
1- add di for "UserManager" and "SignInManager" + add di in startup "AddAuthentication"(added in the vid5)
2- add layout, remove the html code that will be changed with chane of view and add the "@RenderBody" and css file in wwroot
3- register action
4- add register post action
5- add RegisterViewModel for register
6- add view for register action, Add @model RegisterViewModel
  ,edit form and and add asp-for="Register"(Post Action)
  ,replace the name attr in input with asp-for
7- if we want to add a prop configuration inside identity like Email and password  
   we will make a another class which inherit from Identity User "ApplicationUser:IdentityUser",
   and we will update all "identiyUser" to "ApplicationUser" and use ApplicationUser instead
 - dont FORGET to add migration after adding "IsAgree" prop as it represnt a column in IdentitUser tables
8- add the logic in the "Register" action
9- make the default Route pattern: "{controller=Account}/{action=Login}/{id?}"); in the startup
 */
#endregion
...
/*
1- loginAction + view 
2- go to the controllers and add [Authorize]: 
    this will make sure that no one can access this contoller(and views as the view can be accessed be controllers) 
    only if he has logined (Authorized)
3- layout and add the signout button which goes to "Account" contoller
4- add "Signout" action
 */
#endregion
