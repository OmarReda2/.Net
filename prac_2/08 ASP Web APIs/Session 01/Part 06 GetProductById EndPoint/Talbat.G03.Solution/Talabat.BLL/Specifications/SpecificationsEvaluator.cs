    ...
    public class SpecificationsEvaluator<TEntity> where TEntity:BaseEntity
    {
        ...
        //   b. ISpecification (criteria and list of Includes)
        {
            ...
            if (spec.Criteria != null)
                query = query.Where(spec.Criteria); // -after quering this line:
                                                    //  _context.Set<Product>.Where(P => P.id == 1)



            // before quering this line: _context.Set<Product>
            query = spec.Includes.Aggregate(query, (currentQuery, include) => currentQuery.Include(include));
            // - aggregate :its a for loop that add inclucdes to the query (_context.Set<Product>)

            // - after quering the line above:
            // _context.Set<Product>.Include(P => P.ProductType).Include(P => P.ProductBrand)
            // spec.Criteria != null): _context.Set<Product>.Where(P => P.id == 1)
